-- Idempotent re-apply of weddings INSERT policy fix.
-- Migration 003 added this fix but may not have been applied to the live DB.
-- The original policy (auth.uid() = created_by) fails when the browser-side
-- Supabase client session isn't fully propagated after magic link auth.
